...
export default class Utility {
    ...
    static createPerspectiveMatrix (fovy, aspecto, n, f) {
        let q = 1.0 / Math.tan(Utility.toRadians(0.5 * fovy));
        let A = q / aspecto;
        let B = (n + f) / (n - f);
        let C = (2.0 * n * f) / (n - f);
        let datos = [
        ...
        let matriz = new Matrix4x4(datos);
        return matriz;
    }
    ...
    static descargarDesdeUrl (url) {
        return new Promise(resolve => {
            ...
            let mime = Utility.convertirExtensionAMime(extension);
    ...
    static convertirExtensionAMime (extension) {
        let llaves = Object.keys(Utility.asociacionMimeExtension);
        for (let i = 0; i < llaves.length; i++) {
            if (Utility.asociacionMimeExtension[llaves[i]] == extension) {
                return llaves[i];
            }
        }
    }
